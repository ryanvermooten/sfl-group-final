class EmploymentsController < ApplicationController
  ...
  @employment= @gardener.build_employment          
...
# POST /groups/:group_id/group_gardeners.xml
  ...
  @gardener= @group.gardeners.find(params[:gardener_id])
  @employment = @gardener.build_employment employment_params

      if @employment.save
          redirect_to new_group_gardener_grant_path(@group, @gardener)
        #render "employments/new", id:@gardener 
      else
    render "edit"
  end
        #1st argument of redirect_to is an array, in order to build the correct route to the nested resource gardener
     # format.html {redirect_to([@gardener.group, @employments], :notice => 'Garden info was sucessfully saved' )}
      #format.xml {render :xml => @gardener, :status => :created, :location => [@gardener.group,@employments] }
    end
  end
  ...
  def employment_params
    params.require(:employment).permit(:gardener_id, :employed, :employment_type, :earning_power)
  end
